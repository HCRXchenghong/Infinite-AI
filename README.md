...
v2.0.0 是当前阶段的集成提交版本，Infinite Code 正式作为 Infinite-AI 的新增桌面端/Agent 子产品纳入这一版交付范围。它负责桌面工作区、代码 Agent、模型选择、账号登录、套餐额度与用量入口；Infinite-AI 负责提供统一的账号、鉴权、模型、计费、额度同步、错误上报和后台管理能力。Agent 能力目前仍存在已知问题，尤其是工具执行、工作区连续性、归档、审查、MCP、Skill、GitHub 与 Gitee 接入仍需要继续完整验证；前端 UI 也会进入二期优化，重点处理排版密度、响应式、套餐/用量/管理页风格一致性。
...
- Infinite Code：v2.0.0 新增的桌面端/Agent 子产品，接入 Infinite-AI 的登录、订阅、额度、模型、用量、错误上报与后台配置体系。
